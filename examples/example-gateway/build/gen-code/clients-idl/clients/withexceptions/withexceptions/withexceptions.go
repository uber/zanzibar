...
// Encode serializes a ExceptionType1 struct directly into bytes, without going
...
// Decode deserializes a ExceptionType1 struct directly from its Thrift-level
// representation, without going through an intemediary type.
//
// An error is returned if a ExceptionType1 struct could not be generated from the wire
// representation.
...
// Encode serializes a ExceptionType2 struct directly into bytes, without going
// through an intermediary type.
//
...
// Decode deserializes a ExceptionType2 struct directly from its Thrift-level
// representation, without going through an intemediary type.
//
// An error is returned if a ExceptionType2 struct could not be generated from the wire
// representation.
...
func _ExceptionType1_Decode(sr stream.Reader) (*ExceptionType1, error) {
	var v ExceptionType1
	err := v.Decode(sr)
	return &v, err
}

func _ExceptionType2_Decode(sr stream.Reader) (*ExceptionType2, error) {
	var v ExceptionType2
	err := v.Decode(sr)
	return &v, err
}
...
func (v *WithExceptions_Func1_Result) Decode(sr stream.Reader) error {
	...
	for ok {
		...
		case fh.ID == 1 && fh.Type == wire.TStruct:
			v.E1, err = _ExceptionType1_Decode(sr)
			if err != nil {
				return err
			}

		case fh.ID == 2 && fh.Type == wire.TStruct:
			v.E2, err = _ExceptionType2_Decode(sr)
			if err != nil {
				return err
			}
