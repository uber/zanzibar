...
// Encode serializes a NestHeaders struct directly into bytes, without going
// through an intermediary type.
//
// An error is returned if a NestHeaders struct could not be encoded.
func (v *NestHeaders) Encode(sw stream.Writer) error {
	if err := sw.WriteStructBegin(); err != nil {
		return err
	}
...
// Decode deserializes a NestHeaders struct directly from its Thrift-level
// representation, without going through an intemediary type.
//
// An error is returned if a NestHeaders struct could not be generated from the wire
// representation.
func (v *NestHeaders) Decode(sr stream.Reader) error {
...
// Encode serializes a TransHeaders struct directly into bytes, without going
// through an intermediary type.
//
// An error is returned if a TransHeaders struct could not be encoded.
func (v *TransHeaders) Encode(sw stream.Writer) error {
	if err := sw.WriteStructBegin(); err != nil {
		return err
	}
...
func _Wrapped_Decode(sr stream.Reader) (*Wrapped, error) {
	var v Wrapped
	err := v.Decode(sr)
	return &v, err
}

// Decode deserializes a TransHeaders struct directly from its Thrift-level
// representation, without going through an intemediary type.
//
// An error is returned if a TransHeaders struct could not be generated from the wire
// representation.
func (v *TransHeaders) Decode(sr stream.Reader) error {
	...
	for ok {
		...
		case fh.ID == 1 && fh.Type == wire.TStruct:
			v.W1, err = _Wrapped_Decode(sr)
			if err != nil {
				return err
			}
			w1IsSet = true
		case fh.ID == 2 && fh.Type == wire.TStruct:
			v.W2, err = _Wrapped_Decode(sr)
			if err != nil {
				return err
			}
...
// Encode serializes a Wrapped struct directly into bytes, without going
// through an intermediary type.
//
// An error is returned if a Wrapped struct could not be encoded.
func (v *Wrapped) Encode(sw stream.Writer) error {
	if err := sw.WriteStructBegin(); err != nil {
		return err
	}
...
func _NestHeaders_Decode(sr stream.Reader) (*NestHeaders, error) {
	var v NestHeaders
	err := v.Decode(sr)
	return &v, err
}

// Decode deserializes a Wrapped struct directly from its Thrift-level
// representation, without going through an intemediary type.
//
// An error is returned if a Wrapped struct could not be generated from the wire
// representation.
func (v *Wrapped) Decode(sr stream.Reader) error {
	...
	for ok {
		switch {
		case fh.ID == 1 && fh.Type == wire.TStruct:
			v.N1, err = _NestHeaders_Decode(sr)
			if err != nil {
				return err
			}
			n1IsSet = true
		case fh.ID == 2 && fh.Type == wire.TStruct:
			v.N2, err = _NestHeaders_Decode(sr)
			if err != nil {
				return err
			}
